...
func (m *CacheOptions) CloneVT() *CacheOptions {
	...
	if rhs := m.CacheMountExports; rhs != nil {
		tmpContainer := make([]*CacheMountEntry, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v.CloneVT()
		}
		r.CacheMountExports = tmpContainer
	}
	if rhs := m.CacheMountImports; rhs != nil {
		tmpContainer := make([]*CacheMountEntry, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v.CloneVT()
		}
		r.CacheMountImports = tmpContainer
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (m *CacheMountEntry) CloneVT() *CacheMountEntry {
	if m == nil {
		return (*CacheMountEntry)(nil)
	}
	r := new(CacheMountEntry)
	r.ID = m.ID
	r.Type = m.Type
...
func (m *CacheMountEntry) CloneMessageVT() proto.Message {
	return m.CloneVT()
}
...
func (this *CacheOptions) EqualVT(that *CacheOptions) bool {
	...
	if len(this.CacheMountExports) != len(that.CacheMountExports) {
		return false
	}
	for i, vx := range this.CacheMountExports {
		vy := that.CacheMountExports[i]
		if p, q := vx, vy; p != q {
			if p == nil {
				p = &CacheMountEntry{}
			}
	...
	if len(this.CacheMountImports) != len(that.CacheMountImports) {
		return false
	}
	for i, vx := range this.CacheMountImports {
		vy := that.CacheMountImports[i]
		if p, q := vx, vy; p != q {
			if p == nil {
				p = &CacheMountEntry{}
			}
...
func (this *CacheMountEntry) EqualVT(that *CacheMountEntry) bool {
	if this == that {
		return true
	} else if this == nil || that == nil {
		return false
...
func (this *CacheMountEntry) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*CacheMountEntry)
	if !ok {
		return false
	}
	return this.EqualVT(that)
...
func (m *CacheOptions) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.CacheMountImports) > 0 {
		for iNdEx := len(m.CacheMountImports) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.CacheMountImports[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
	...
	if len(m.CacheMountExports) > 0 {
		for iNdEx := len(m.CacheMountExports) - 1; iNdEx >= 0; iNdEx-- {
			size, err := m.CacheMountExports[iNdEx].MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
...
func (m *CacheMountEntry) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *CacheMountEntry) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *CacheMountEntry) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
...
func (m *CacheOptions) SizeVT() (n int) {
	...
	if len(m.CacheMountExports) > 0 {
		for _, e := range m.CacheMountExports {
			l = e.SizeVT()
			n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
		}
	}
	if len(m.CacheMountImports) > 0 {
		for _, e := range m.CacheMountImports {
			l = e.SizeVT()
			n += 1 + l + protohelpers.SizeOfVarint(uint64(l))
		}
	}
...
func (m *CacheMountEntry) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *CacheOptions) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 6:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field CacheMountExports", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.CacheMountExports = append(m.CacheMountExports, &CacheMountEntry{})
			if err := m.CacheMountExports[len(m.CacheMountExports)-1].UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 7:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field CacheMountImports", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.CacheMountImports = append(m.CacheMountImports, &CacheMountEntry{})
			if err := m.CacheMountImports[len(m.CacheMountImports)-1].UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
...
func (m *CacheMountEntry) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: CacheMountEntry: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: CacheMountEntry: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
